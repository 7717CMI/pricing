...
export function MatrixHeatmap({ title, height = 600 }: MatrixHeatmapProps) {
  ...
  const matrixData = useMemo(() => {
    ...
    // Helper to convert year to month label within useMemo
    const dataStartYear = data.metadata.start_year
    const toMonth = (year: number) => `Month ${year - dataStartYear + 1}`
  ...
  const dataStartYear = data.metadata.start_year
  const yearToMonth = (year: number) => `Month ${year - dataStartYear + 1}`
  ...
  return (
    <div className="w-full">
      <div className="mb-4">
        ...
        <p className="text-sm text-black mt-1">
          Months: {yearToMonth(startYear)} - {yearToMonth(endYear)} | {filters.dataType === 'value' ? 'Values' : 'Volume'} in {valueUnit}
        </p>
      </div>
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-black">{yearToMonth(hoveredCell.year)}</span>
            </div>
            <div className="flex items-center justify-between">
      ...
      <div className="mt-4 text-center text-sm text-black">
        Comparing {matrixData.geographyYears.length} geography-month combinations × {matrixData.segments.length} segments
      </div>
    </div>
  )
}
