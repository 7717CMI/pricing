...
export function ComparisonTable({ title, height = 600 }: ComparisonTableProps) {
  ...
  const yearToMonth = (yr: number) => `Month ${yr - startYear + 1}`
    ...
    <div className="w-full min-w-0 overflow-hidden">
      <div className="mb-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-black mt-1">
            Month: {yearToMonth(year)} | Values in {valueUnit}
          </p>
        </div>
        <button
          onClick={exportToCSV}
      ...
      <div className="mt-4 text-center text-sm text-black">
        Showing {sortedData.length} records | {yearToMonth(filters.yearRange[0])} - {yearToMonth(filters.yearRange[1])}
      </div>
    </div>
  )
}
